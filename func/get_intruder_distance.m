function [distance,interact] = get_intruder_distance(mouse_bodyparts,juv_bodyparts,varargin)
dist_thresh = 5/10*80; % 5 cm in pixels; For miniscope: 10 cm per 80 pixels (1.25mm per pixel); For Staar box 6.1: 0.6 mm per pixel
mintime = 10; % frames
ignorepts = 5; % ignore small interaptions
discard_frames = []; % set these frames to nan
...
mouse_var = {'Nose_1'}; juv_var = {'Nose_2','Ear_left_2','Ear_right_2','Tail_base_2','Center_2'};
[mouse_position] =  get_bodyparts_position(mouse_bodyparts,mouse_var);
[juv_position] =  get_bodyparts_position(juv_bodyparts,juv_var);
if ~isempty(discard_frames)
    mouse_position(discard_frames,:) = nan;
end
distance.mouse_juv = arrayfun(@(x) pdist([mouse_position(x,:);juv_position(x,:)]),1:size(mouse_position,1));

%% nose(test mouse)-nose(juvenile) distance
% use implant position as test mouse nose
mouse_var = {'Nose_1'}; juv_var = {'Nose_2','Ear_left_2','Ear_right_2'};
[mouse_position] =  get_bodyparts_position(mouse_bodyparts,mouse_var);
[juv_position] =  get_bodyparts_position(juv_bodyparts,juv_var);
distance.nose_nose = arrayfun(@(x) pdist([mouse_position(x,:);juv_position(x,:)]),1:size(mouse_position,1));

%% nose (test mouse) - body(juvenile) distance
mouse_var = {'Nose_1'}; juv_var = {'Center_2'};
[mouse_position] =  get_bodyparts_position(mouse_bodyparts,mouse_var);
[juv_position] =  get_bodyparts_position(juv_bodyparts,juv_var);
distance.nose_butt = arrayfun(@(x) pdist([mouse_position(x,:);juv_position(x,:)]),1:size(mouse_position,1));

%% butt (test mouse) - nose (juvenile) distance
mouse_var = {'Tail_base_1'}; juv_var = {'Nose_2','Ear_left_2','Ear_right_2'};
[mouse_position] =  get_bodyparts_position(mouse_bodyparts,mouse_var);
[juv_position] =  get_bodyparts_position(juv_bodyparts,juv_var);
distance.butt_nose = arrayfun(@(x) pdist([mouse_position(x,:);juv_position(x,:)]),1:size(mouse_position,1));

%% butt (test mouse) - butt (juvenile) distance
mouse_var = {'Tail_base_1'}; juv_var = {'Center_2'};
[mouse_position] =  get_bodyparts_position(mouse_bodyparts,mouse_var);
[juv_position] =  get_bodyparts_position(juv_bodyparts,juv_var);
distance.butt_butt = arrayfun(@(x) pdist([mouse_position(x,:);juv_position(x,:)]),1:size(mouse_position,1));

distance = structfun(@(x)median(x,ignorepts),distance,'un',0);
...
interact.nose_nose = init_trace; 
interact.nose_nose(distance.nose_nose<dist_thresh&distance.nose_nose<distance.butt_nose) = 1;
interact.nose_butt =init_trace; 
interact.nose_butt(distance.nose_butt<dist_thresh&distance.nose_butt<distance.butt_butt) = 1;
interact.sniffs = init_trace;
interact.sniffs(interact.nose_nose |interact.nose_butt ) = 1;
interact.proximal = init_trace; 
interact.proximal(distance.mouse_juv<dist_thresh) = 1;
%% approaching episodes
% 1. nose-nose and nose-butt distance should be decreasing
% 2. nose-nose distance should be shorter than butt-nose distance
temp = [diff(distance.nose_nose) 0];
...
interact.approaching(temp_approach_trace&distance.nose_nose<distance.butt_nose) = 1;
...
interact.escaping (temp_escape_trace&distance.nose_nose>distance.butt_nose&distance.nose_nose>dist_thresh) = 1;
